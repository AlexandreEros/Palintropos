...
extern "C" __global__
void generate_sph_harm_basis(const double* lat, const double* lon,
    ...
    double theta = lon[idx];           // Azimuthal
    ...
    // Y_matrix is row-major: [point_0_coeffs, point_1_coeffs, ...]
    ...
    for (int m = 0; m <= l_max; m++) {
        ...
        for (int l = m; l <= l_max; l++) {
            ...
            // Using lgamma for large factorials
            double log_fact_diff = lgamma(l - m + 1.0) - lgamma(l + m + 1.0);
            double norm = sqrt( (2.0 * l + 1.0) / (4.0 * M_PI) * exp(log_fact_diff) );

            // P_curr already includes (-1)^m phase from P_m^m computation

            double real_part = p_curr * norm * cos(m * theta);
            double imag_part = p_curr * norm * sin(m * theta);

            // Linear index
            int basis_idx = l * (l + 1) / 2 + m;
            row_ptr[basis_idx] = complex<double>(real_part, imag_part);
