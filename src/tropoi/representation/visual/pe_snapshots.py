...
the shared :func:`tropoi.representation.visual.timeline.render_snapshot_product`, so a
...
* it reuses the model's own spherical-harmonic synthesis and the shared lat-lon
  view adapter (via :mod:`tropoi.representation.visual.pe` — no second
  synthesis implementation), the shared declarative
  :mod:`tropoi.representation.visual.specs` panels, the timeline's cross-frame
...
* color scaling is symmetric about zero and resolved ONCE per variable across
  ...
  :class:`~tropoi.representation.visual.normalization.NormalizationPolicy` behavior),
...
def _surface_pressure_anomaly(model, lnps_coeffs_2d) -> np.ndarray:
    ...
    """
    from tropoi.representation.visual.pe import _synthesize
...
def build_pe_snapshot_figure(model, fields: PESnapshotFields, *,
    ...
    """
    from tropoi.representation.visual.pe import _view_field
    from tropoi.representation.visual.normalization import NormalizationPolicy
    from tropoi.representation.visual.specs import (FigureSpec, PanelPlacement,
...
def build_pe_snapshot_timeline(model, out_dir: pathlib.Path | str, *,
    ...
    One :class:`~tropoi.representation.visual.timeline.FigureFrame` per stored time.
    ...
    """
    from tropoi.representation.visual.pe import _load_pe_coeffs
...
def build_pe_snapshot_timeline_from_data(model, coefficients, times_seconds,
    ...
    the in-run product.
    """
    from tropoi.representation.visual.pe import _validate_pe_coeffs
    from tropoi.representation.visual.timeline import FigureFrame, FigureTimeline
...
def render_pe_snapshots(model, out_dir: pathlib.Path | str, *,
    ...
    from tropoi.representation.visual.timeline import render_snapshot_product
